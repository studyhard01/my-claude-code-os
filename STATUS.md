...
- **최종 갱신**: 2026-07-14 (수집 다변화 — 워크넷 어댑터 구현, 계약 승인 게이트 첫 실전 통과)
- **현재 마일스톤**: **M1 — 공고 모아보기 MVP** (직무 범위: 개발직군 한정)
- **한 줄 요약**: 수집 소스 2개 체제(사람인 승인대기 + **워크넷 공공 API — 키만 발급하면 즉시 실수집**). 실 데이터 검증이 더는 사람인 승인에 묶여 있지 않다.
...
- **수집 다변화 계약 + 워크넷 어댑터 (2026-07-14, PR #2)** — 결정 3건: ① M1 제2 소스 워크넷 공공 API ② M2 축 = 회사 구독 라이트(12.9 계약 초안) ③ 채용 플랫폼 크롤링 금지 명문화(잡코리아 v 사람인 대법 판례). 구현: RawJob 정규 필드 승격(12.8 확정), `WorknetAdapter`(XML 자체 파싱·fetchFn 주입·5콜 상한·필드명 불확실성은 `WORKNET_FIELD` 상수로 격리), fixture 6건(FULL 3/PARTIAL 3), `COLLECT_SOURCE=worknet[-fixture]`, 테스트 29개 통과·idempotent 확인. **계약 승인 게이트(Draft PR) 첫 실전 — 기획→Draft PR→사용자 승인→구현 순서가 실제로 돌았다**
...
1. **공공데이터포털(data.go.kr) 가입 → 워크넷 채용정보 API 활용신청(자동승인) → `WORKNET_API_KEY` 설정** → `COLLECT_SOURCE=worknet npm run collect` 첫 실수집. 어댑터 주석의 체크리스트(엔드포인트·태그명·직종코드·상시채용 표현·본문 유무)를 실응답과 대조
2. **M1 완료 기준 검증**: "온보딩→피드→북마크"가 워크넷 실 데이터로 끝까지 도는가 + name 키워드 매핑 커버리지 점검
3. 사람인 API 승인 확인 시 `COLLECT_SOURCE=saramin` 병행 전환(12.8 "승인 후 최종 검증") — 이제 보조 트랙
