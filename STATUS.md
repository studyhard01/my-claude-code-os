...
- **최종 갱신**: 2026-07-14 (제2 소스 = 잡알리오 실연동 — 워크넷 개인 발급 불가로 피벗, PR #3 머지)
- **현재 마일스톤**: **M1 — 공고 모아보기 MVP** (직무 범위: 개발직군 한정)
- **한 줄 요약**: 수집 소스 2개 체제(사람인 승인대기 + **잡알리오 공공기관 채용 — 개인 개발키 발급·실응답 검증 완료**). 실 데이터 검증이 더는 사람인 승인에 묶여 있지 않다. 잡알리오 라이브 수집만 남았다(현 작업 환경 `.env` 에 `ALIO_API_KEY` 설정 필요 — 키는 이전 작업 환경에만 있음).
...
1. **잡알리오 라이브 수집** — `COLLECT_SOURCE=alio npm run collect` 로 실 API 수집→upsert→피드 노출. ⚠ `ALIO_API_KEY` 가 현 작업 환경 `.env` 에 없음(발급 키는 data.go.kr 마이페이지에서 확인 가능, `.env` 는 git 미포함이라 환경 간 이동 안 됨) → 키 설정 후 실행. fixture end-to-end 검증은 완료 상태라 실수집만 돌리면 됨
2. **M1 완료 기준 검증**: "온보딩→피드→북마크"가 잡알리오 실 데이터로 끝까지 도는가
3. 사람인 API 승인 확인 시 `COLLECT_SOURCE=saramin` 병행 전환(12.8 "승인 후 최종 검증") — 주력(민간 광역) 소스
4. (M2) 잡알리오 공고 대부분이 PARTIAL 로 숨는 문제 — 직무 분류를 "정보통신/IT일반" 같은 coarse 값으로 받을지 기획 재검토(회사 구독 재편과 함께)
