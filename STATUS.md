...
- **한 줄 요약**: 수집 소스 2개 체제(사람인 승인대기 + **잡알리오 공공기관 채용 — 개인 개발키 발급·실응답 검증 완료**). 실 데이터 검증이 더는 사람인 승인에 묶여 있지 않다. 잡알리오 라이브 수집만 남았고, 사내망 TLS 때문에 비사내망 환경에서 진행 예정.
...
1. **[후속·비사내망 환경] 잡알리오 라이브 수집** — `COLLECT_SOURCE=alio npm run collect` 로 실 API 수집→upsert→피드 노출. **이 PC 는 사내망(Somansa) TLS 가로채기로 외부 API 호출이 막힐 수 있어 보류** — 사내망이 아닌 환경에서 진행. 키(`ALIO_API_KEY`)는 이미 `.env` 에 있고 fixture 로 end-to-end 검증은 끝난 상태라, 실수집만 돌리면 됨
