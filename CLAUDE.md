...
- **이 저장소의 실질 main = `studyhard01` 브랜치다.** 스터디 공유 저장소라 default branch `main` 은 공용(참가자별 브랜치 병렬 존재). 새 브랜치 분기·PR base·병합 후 복귀는 전부 `studyhard01` 로 하고, `main` 에는 checkout·push 하지 않는다 (2026-07-19 실측 — main 을 열었다가 병합 실패로 오인).
